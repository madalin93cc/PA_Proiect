...
public class Regina extends Piesa{
    ...
    public Regina(int x, int y, int side){ 
        setPozitie(new Pair(x, y));
        setSide(side);
    }
    ...
    public boolean muta(Tabla t, int i, int j, boolean q){
        ...
        if ( (Math.abs(i-x_actual) ==0) || (Math.abs(j-y_actual)== 0) || ((Math.abs(i-x_actual)) == Math.abs(j-y_actual)) ){
            int directie=0;
            if ((x_actual < i) && (y_actual == j)) directie = 1;
            else if ((x_actual == i) && (y_actual > j)) directie = 3;
                else if ((x_actual > i) && (y_actual == j)) directie = 5;
                    else if ((x_actual == i) && (y_actual < j)) directie = 7;
                        else if ((x_actual < i) && (y_actual > j)) directie = 2;
                            else if ((x_actual > i) && (y_actual > j)) directie = 4;
                                else if ((x_actual > i) && (y_actual < j)) directie = 6;
                                    else if ((x_actual < i) && (y_actual < j)) directie = 8;
            boolean ok = true;
            switch (directie){
                case 1:{
    ...
    public Regina clona(){
        return new Regina(this.getPozitie().getX(),this.getPozitie().getY(), this.getSide());
    }
